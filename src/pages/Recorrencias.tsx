...
import api from '../api';
import { Plus, Trash2, RefreshCw, Calendar, DollarSign, TrendingUp, TrendingDown, AlertCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion'; // ✅ Motion Principles
...
export default function Recorrencias() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Calcula a primeira execução para hoje ou amanhã
      const hoje = new Date();
      ...
      // Reset e recarrega
      setFormData({ descricao: '', tipo: 'DESPESA', valor: '', frequencia: 'MENSAL', categoria: '' });
    ...
    <div className="space-y-6 text-slate-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div 
          ...
          className="bg-[#0f172a] p-5 rounded-lg border border-slate-800 h-fit space-y-4"
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`flex items-center justify-center gap-1 py-2 rounded border transition-all ${
                    formData.tipo === 'RECEITA' 
                      ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' 
                      : 'bg-[#1e293b]/40 border-slate-700 text-slate-400 hover:border-slate-600'
                  }`}
                >
            ...
            <div>
              ...
              <input 
                type="text" 
                placeholder="Ex: Aluguel do Ponto, Assinatura SaaS"
                value={formData.descricao}
                onChange={e => setFormData({ ...formData, descricao: e.target.value })}
                className="w-full bg-[#1e293b]/40 border border-slate-800 rounded-lg px-3 py-2.5 text-slate-300 focus:outline-none focus:border-cyan-500/50 h-10" 
                required 
              />
            </div>
            ...
            <button 
              type="submit" 
              disabled={submitting}
              className="w-full bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-semibold py-2.5 rounded-lg flex items-center justify-center gap-1.5 transition-colors mt-2"
            >
        ...
        <motion.div 
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          className="bg-[#0f172a] p-5 rounded-lg border border-slate-800 lg:col-span-2 space-y-4"
        >
          <div className="flex items-center gap-2 border-b border-slate-800 pb-2">
              ...
              <AnimatePresence>
                {recorrencias.map((rec) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    className="flex items-center justify-between p-4 bg-[#1e293b]/30 border border-slate-800 rounded-lg hover:border-slate-700 transition-colors group"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-cyan-400 mt-1">
                          Próxima execução: {new Date(rec.proximaExecucao).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <button 
                        onClick={() => handleCancelar(rec.id)}
                        className="flex items-center gap-1 text-[10px] text-slate-500 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                      >
