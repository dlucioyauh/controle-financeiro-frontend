...
import api from '../api';
import Despesas from '../components/Despesas'; // ← importação adicionada
...
export default function Financeiro() {
  ...
  const carregarDados = async () => {
    setCarregando(true);
    try {
      // Buscar estatísticas de vendas e lista de despesas
      ...
      const despesasFiltradas = (despesasRes.data || []).filter((d: any) => {
        ...
        const dataFormatada = d.data.slice(0, 10); // garante 'YYYY-MM-DD'
      ...
      // --- Transações unificadas (vendas + despesas) ---
                  ...
                  {transacoes.map((t, i) => (
                    <tr key={i} className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="py-2 px-3">
                        {t.tipo === 'venda' ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-red-400">
                            <ArrowDownRight size={10} /> Despesa
                          </span>
                        )}
                      </td>
                      ...
                      <td className={`py-2 px-3 text-right font-bold ${
                        t.valor >= 0 ? 'text-emerald-400' : 'text-red-400'
                      }`}>
